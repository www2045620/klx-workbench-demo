            ...
            function ($timeout,
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        attrs.iconExpand = attrs.iconExpand ? attrs.iconExpand : 'icon-plus  glyphicon glyphicon-plus  ';
                        attrs.iconCollapse = attrs.iconCollapse ? attrs.iconCollapse : 'icon-minus glyphicon glyphicon-minus ';
                        attrs.iconLeaf = attrs.iconLeaf ? attrs.iconLeaf : 'icon-file  glyphicon glyphicon-file  ';
                        attrs.sortedAsc = attrs.sortedAsc ? attrs.sortedAsc : 'icon-file  glyphicon glyphicon-chevron-up  ';
                        attrs.sortedDesc = attrs.sortedDesc ? attrs.sortedDesc : 'icon-file  glyphicon glyphicon-chevron-down ';
                        ...
                        //状态更改调用
						scope.stateChange=function(branch){
							...
							if(scope.onStateChange){
								scope.onStateChange({
									...
									"callback" : function(response){
                                		if(!response.success){
                                			response.branch.state = !response.branch.state;
                                		}
                                	}
								});						
							}
						...
						//显示排序输入框
						scope.rankShow=function(e,branch){
							this.row.branch.rankInput=true;
						}
						//输入框失去焦点事件
						scope.rankInputblur=function(e){
							//this.row.branch.sort = e.target.value;
							var defaultVal = this.row.branch.sort;
							var fixedVal = e.target.value;
							if(fixedVal&&fixedVal!=defaultVal){
								if (scope.onTreeRank) {
	                                scope.onTreeRank({
	                                	...
	                                	"callback" : function(response){
	                                		if(response.success){
	                                			response.branch.sort = fixedVal;
	                                    		response.branch.rankInput=false;
	                							sort_recursive(scope.treeData,response.sort, false);
	                                		}
	                                	}
	                                });
	                            }
							}else{
								e.target.value = this.row.branch.sort;
								this.row.branch.rankInput=false;
							}
							
						}
						/**
						 * 操作列事件
						 * @param operate_type 操作类型（用户自定义）
						 */
						scope.branchOperation = function(operate_type){
							if(scope.onBranchOperation){
								scope.onBranchOperation({
									"branch" : this.row.branch,
                        ...
                        if (scope.treeControl != null) {
                            if (angular.isObject(scope.treeControl)) {
                                ...
                                tree.sort_recursive = function(elements, col, descending){
                                	sort_recursive(elements, col, descending);
                                }
                                tree.remove_selected_branch = function(b){
                                	var parent_branch = tree.get_parent_branch(b);
                                	var treeDataArray = [];
                                	if(!parent_branch){
                                		treeDataArray = scope.treeData;
                                	}else{
                                		treeDataArray = parent_branch.children;
                                	}
                                	angular.forEach(treeDataArray,function(data,index,array){
                                		if(b.uid == data.uid){
                                			treeDataArray.splice(index,1);
                                		}
                                	});
